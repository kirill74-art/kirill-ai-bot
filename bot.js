...
if (!BOT_TOKEN || !OPENROUTER_API_KEY) {
    console.error('❌ Не найдены переменные BOT_TOKEN или OPENROUTER_API_KEY');
    process.exit(1);
}
                ...
                messages: [
                    {
                        ...
                        content:
                            ...
                            'Отвечай понятно, кратко и на русском языке, если пользователь пишет на русском.'
                    },
                    ...user.history
                ]
            })
...
bot.action('profile', async (ctx) => {
    ...
    await ctx.reply(
        ...
        `🆔 ID: \`${ctx.from.id}\`\n` +
        `⭐ Статус: ${status}\n` +
        `📨 Запросов сегодня: ${user.requests}/${FREE_LIMIT}`,
        {
            parse_mode: 'Markdown',
            ...mainMenu()
        }
